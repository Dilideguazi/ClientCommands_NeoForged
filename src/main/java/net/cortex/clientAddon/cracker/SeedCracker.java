...
//        Minecraft.getInstance().getConnection().send(new ServerboundMovePlayerPacket(0, 90, true));
        Objects.requireNonNull(Minecraft.getInstance().getConnection()).send(new ServerboundMovePlayerPacket.Rot(0, 90, true)); // point to correct location
        for (int i = 0; i < 20; i++) {
    ...
    public static void onEntityCreation(ClientboundAddEntityPacket packet) {
        if (packet.getType() == EntityType.ITEM && Configs.playerCrackState == PlayerRandCracker.CrackState.CRACKING) {
            if (SeedCracker.expectedItems > 0) {
                ...
                SeedCracker.bits[20-SeedCracker.expectedItems]= value;//could be improved
