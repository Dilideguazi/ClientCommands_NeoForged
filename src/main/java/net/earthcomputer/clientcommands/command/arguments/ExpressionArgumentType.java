...
public class ExpressionArgumentType implements ArgumentType<ExpressionArgumentType.Expression> {
    ...
    public static class FunctionExpression extends Expression {
        ...
        @Override
        public Component getParsedTree(int depth) {
            MutableComponent argumentsComponent = Component.literal("");
            boolean first = true;
            for (Expression argument : this.arguments) {
                if (first) {
                    first = false;
                } else {
                    argumentsComponent.append(", ");
                }

                argumentsComponent.append(argument.getParsedTree(depth + 1));
            }

            return getDepthStyled(depth, Component.translatable("commands.ccalc.parse.function", this.type, argumentsComponent));
        }
