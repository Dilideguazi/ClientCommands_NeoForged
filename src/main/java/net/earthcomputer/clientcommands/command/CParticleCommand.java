        ...
        switch (source.getClient().options.particles().get()) {
            ...
            default:
                if (count == 0) {
                    ...
                    final RandomSource random = Objects.requireNonNull(source.getClient().getConnection()).getLevel().random;
                ...
                source.sendFeedback(Component.translatable("commands.cparticle.success",
                        Objects.requireNonNull(ForgeRegistries.PARTICLE_TYPES.getKey(parameters.getType())).toString()));
