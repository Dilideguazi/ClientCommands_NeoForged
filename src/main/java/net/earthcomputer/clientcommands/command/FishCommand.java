...
import static net.earthcomputer.clientcommands.command.arguments.ItemAndEnchantmentsPredicateArgument.ItemAndEnchantmentsPredicate;
import static net.earthcomputer.clientcommands.command.arguments.ItemAndEnchantmentsPredicateArgument.itemAndEnchantmentsPredicate;
...
public class FishCommand {
        ...
        dispatcher.register(literal("cfish")
            ...
            .then(literal("add-goal")
                .then(argument("goal", withString(clientItemPredicate(context)))
                    .executes(ctx -> addGoal(ctx.getSource(), getWithString(ctx, "goal", ClientItemPredicate.class)))))
    ...
    private static int addGoal(FabricClientCommandSource source, Result<ClientItemPredicate> goal) throws CommandSyntaxException {
        if (!Configs.fishingManipulation.isEnabled()) {
            throw NEED_FISHING_MANIPULATION_EXCEPTION.create();
        }
    ...
    private static int addEnchantedGoal(FabricClientCommandSource source, Result<ItemAndEnchantmentsPredicate> stringAndItemAndEnchantments) throws CommandSyntaxException {
        if (!Configs.fishingManipulation.isEnabled()) {
            throw NEED_FISHING_MANIPULATION_EXCEPTION.create();
        }
