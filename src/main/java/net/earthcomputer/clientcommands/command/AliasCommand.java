...
public class AliasCommand {
    ...
    private static final Path ALIAS_PATH = FMLPaths.CONFIGDIR.get().resolve("clientcommands").resolve("alias_list.json");
    ...
    public static String convertAlias(String command) {
        if (command == null || aliasMap == null || aliasMap.isEmpty()) {
            return command;
        }

        List<String> keys = new ArrayList<>(aliasMap.keySet());
        keys.sort((a, b) -> b.length() - a.length());

        StringBuilder regexBuilder = new StringBuilder("^(");
        for (int i = 0; i < keys.size(); i++) {
            if (i > 0) regexBuilder.append('|');
            regexBuilder.append(Pattern.quote(keys.get(i)));
        }
        ...
        String replacement = aliasMap.get(matchedKey);
    ...
    private static int addAlias(FabricClientCommandSource source, String key, String command) throws CommandSyntaxException {
        ...
        for (CommandDispatcher<FabricClientCommandSource> dispatcher : new CommandDispatcher[] { ClientCommandManager.getActiveDispatcher(), Objects.requireNonNull(Minecraft.getInstance().getConnection()).getCommands() }) {
            dispatcher.register(literal(key)
    ...
    private static int removeAlias(FabricClientCommandSource source, String key) throws CommandSyntaxException {
        if (aliasMap.containsKey(key)) {
            ...
            BrigadierRemover.of(Objects.requireNonNull(Minecraft.getInstance().getConnection()).getCommands()).get(key).remove();
            aliasMap.remove(key);
        } else {
