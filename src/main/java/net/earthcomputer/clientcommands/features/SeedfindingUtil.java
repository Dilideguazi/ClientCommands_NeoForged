...
public class SeedfindingUtil {
    ...
    public static ItemStack fromSeedfindingItem(com.seedfinding.mcfeature.loot.item.ItemStack stack) {
        Item item = ForgeRegistries.ITEMS.getValue(ResourceLocation.tryParse(stack.getItem().getName()));
        ...
        for (var enchAndLevel : stack.getItem().getEnchantments()) {
            Enchantment enchantment = ForgeRegistries.ENCHANTMENTS.getValue(ResourceLocation.tryParse(enchAndLevel.getFirst()));
            if (enchantment == null) {
                continue;
            }
