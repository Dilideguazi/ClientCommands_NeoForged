...
public class RenderSettings {
    ...
    public static void preRenderEntities() {
        ...
        FabricClientCommandSource source = (FabricClientCommandSource) new ClientSuggestionProvider(Objects.requireNonNull(client.getConnection()), client);
...
//                List<UUID> entities = filter.getA().getEntities(source).stream().map(Entity::getUUID).collect(Collectors.toList());
                List<UUID> entities = filter.getA().getEntities(source).stream().map(Entity::getUUID).toList();
                if (filter.getB()) {
