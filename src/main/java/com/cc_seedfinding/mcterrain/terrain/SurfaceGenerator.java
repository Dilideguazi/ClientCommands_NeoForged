package com.cc_seedfinding.mcterrain.terrain;
...
import com.cc_seedfinding.mccore.util.data.Triplet;
import com.cc_seedfinding.mccore.util.pos.BPos;
import com.cc_seedfinding.mccore.version.MCVersion;
import com.cc_seedfinding.mcnoise.noise.NoiseSampler;
import com.cc_seedfinding.mcnoise.perlin.OctavePerlinNoiseSampler;
import com.cc_seedfinding.mcnoise.perlin.PerlinNoiseSampler;
import com.cc_seedfinding.mcnoise.simplex.OctaveSimplexNoiseSampler;
...
import com.cc_seedfinding.mcterrain.utils.NoiseSettings;
...
import static com.cc_seedfinding.mcnoise.utils.MathHelper.maintainPrecision;
import static com.cc_seedfinding.mcterrain.utils.MathHelper.*;
...
public abstract class SurfaceGenerator extends TerrainGenerator {
	...
	private void replaceBiomeBlocks(Block[] buffer, int x, int z, ChunkRand rand) {
		int y = (int) com.cc_seedfinding.mcterrain.utils.MathHelper.clamp(this.getHeightOnGround(x, z), this.getMinWorldHeight(), this.getMaxWorldHeight() - 1);
	...
	public double applyJigsawToNoise(double noise, BPos bPos, List<Pair<Supplier<Integer>, BlockBox>> jigsawBoxes, List<BPos> jigsawJunction) {
		noise = com.cc_seedfinding.mcterrain.utils.MathHelper.clamp(noise / 200.0D, -1.0D, 1.0D);
...
//			noise = noise * 24.575625D - 2.0D;
	...
	private static double computeContribution(int x, int y, int z) {
		...
		double d4 = -d1 * com.cc_seedfinding.mcterrain.utils.MathHelper.fastInvSqrt(d2 / 2.0D + d0 / 2.0D) / 2.0D;
		return d4 * d3;
	}
