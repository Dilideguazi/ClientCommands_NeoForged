...
public final class WaypointRenderingHelper {
    ...
    private static void extractWaypointBoxes(LevelExtractionContext context) {
        ...
        ((FabricRenderState) context.levelState()).setData(WAYPOINT_LIST_KEY, waypointStates);
    }

    private static void renderWaypointBoxes(LevelRenderContext context) {
        List<WaypointState> waypoints = ((FabricRenderState) context.levelState()).getData(WAYPOINT_LIST_KEY);
        if (waypoints == null) {
            return;
        }
