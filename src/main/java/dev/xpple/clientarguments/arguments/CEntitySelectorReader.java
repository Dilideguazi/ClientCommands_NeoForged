...
public class CEntitySelectorReader {
	...
	public static final DynamicCommandExceptionType UNKNOWN_SELECTOR_EXCEPTION = new DynamicCommandExceptionType(selectorType -> Component.translatable("argument.entity.selector.unknown", selectorType));
	...
	public static final DynamicCommandExceptionType VALUELESS_EXCEPTION = new DynamicCommandExceptionType(option -> Component.translatable("argument.entity.options.valueless", option));
	...
	private BiConsumer<Vec3, List<? extends Entity>> sorter = ARBITRARY;
	...
	public CEntitySelector build() {
		...
		if (this.dx != null || this.dy != null || this.dz != null) {
			...
			box = new AABB(-d, -d, -d, d + 1.0, d + 1.0, d + 1.0);
		} else {
			box = null;
		}
		Function<Vec3, Vec3> d = this.x == null && this.y == null && this.z == null ? pos -> pos : pos -> new Vec3(this.x == null ? pos.x : this.x, this.y == null ? pos.y : this.y, this.z == null ? pos.z : this.z);
	...
	private void buildPredicate() {
		...
		if (!this.levelRange.isAny()) {
			this.predicate = this.predicate.and(entity -> {
				...
				return this.levelRange.matches(((ServerPlayer)entity).experienceLevel);
			});
		}
	}

	private Predicate<Entity> rotationPredicate(WrappedMinMaxBounds angleRange, ToDoubleFunction<Entity> entityToAngle) {
		double d = Mth.wrapDegrees(angleRange.getMin() == null ? 0.0f : angleRange.getMin());
		double e = Mth.wrapDegrees(angleRange.getMax() == null ? 359.0f : angleRange.getMax());
		return entity -> {
