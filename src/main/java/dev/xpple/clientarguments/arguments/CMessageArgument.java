...
public class CMessageArgument implements SignedArgument<CMessageArgument.Message> {
	...
	public static Component getMessage(final CommandContext<FabricClientCommandSource> context, final String name) throws CommandSyntaxException {
		Message message = context.getArgument(name, Message.class);
	...
	public <S> Message parse(final StringReader stringReader, final @Nullable S source) throws CommandSyntaxException {
		return Message.parseText(stringReader, CEntitySelectorParser.allowSelectors(source));
	}
	...
	public record Message(String text, Part[] parts) {
		...
		public static Message parseText(StringReader stringReader, boolean allowSelectors) throws CommandSyntaxException {
			...
			while (true) {
				...
				list.add(new Part(j - cursor, stringReader.getCursor() - cursor, entitySelector));
			}
		}
	}
