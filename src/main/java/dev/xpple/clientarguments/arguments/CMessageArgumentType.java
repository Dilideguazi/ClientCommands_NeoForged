...
public class CMessageArgumentType implements ArgumentType<CMessageArgumentType.MessageFormat> {
	...
	public static Component getCMessage(CommandContext<FabricClientCommandSource> context, String name) throws CommandSyntaxException {
	...
	public Component toText(MessageFormat messageFormat) {
		return Component.literal(messageFormat.getContents());
	}

	public CompletableFuture<Component> decorate(FabricClientCommandSource source, MessageFormat messageFormat) throws CommandSyntaxException {
	...
	public static class MessageFormat {
		...
		CompletableFuture<Component> decorate(FabricClientCommandSource source) throws CommandSyntaxException {
			Component text = this.format(source);
		...
		public Component format(FabricClientCommandSource source, boolean canUseSelectors) throws CommandSyntaxException {
			if (this.selectors.length != 0 && canUseSelectors) {
				MutableComponent mutableText = Component.literal(this.contents.substring(0, this.selectors[0].getStart()));
