...
public class CScoreboardSlotArgumentType implements ArgumentType<Integer> {
	...
	public static final DynamicCommandExceptionType INVALID_SLOT_EXCEPTION = new DynamicCommandExceptionType(name -> Component.translatable("argument.scoreboardDisplaySlot.invalid", name));
	...
	public Integer parse(final StringReader stringReader) throws CommandSyntaxException {
		...
		int slotId = Scoreboard.getDisplaySlotByName(string);
		if (slotId == -1) {
	...
	public <S> CompletableFuture<Suggestions> listSuggestions(final CommandContext<S> context, final SuggestionsBuilder builder) {
		return SharedSuggestionProvider.suggest(Scoreboard.getDisplaySlotNames(), builder);
	}
