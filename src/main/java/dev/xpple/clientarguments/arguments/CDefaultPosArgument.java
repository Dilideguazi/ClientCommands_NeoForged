...
public class CDefaultPosArgument implements CPosArgument {
	...
	public CDefaultPosArgument(WorldCoordinate x, WorldCoordinate y, WorldCoordinate z) {
		this.x = x;
		this.y = y;
		this.z = z;
	}
	...
	public static CDefaultPosArgument absolute(double x, double y, double z) {
		return new CDefaultPosArgument(new WorldCoordinate(false, x), new WorldCoordinate(false, y), new WorldCoordinate(false, z));
	}

	public static CDefaultPosArgument absolute(Vec2 vec) {
		return new CDefaultPosArgument(new WorldCoordinate(false, (double)vec.x), new WorldCoordinate(false, (double)vec.y), new WorldCoordinate(true, 0.0D));
	}

	public static CDefaultPosArgument zero() {
		return new CDefaultPosArgument(new WorldCoordinate(true, 0.0D), new WorldCoordinate(true, 0.0D), new WorldCoordinate(true, 0.0D));
	}
