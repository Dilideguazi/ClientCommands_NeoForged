...
extends AbstractConfigCommand<FabricClientCommandSource> {
    ...
    protected int comment(FabricClientCommandSource source, String config, String comment) {
        ...
        source.sendFeedback(Component.nullToEmpty(comment));
        return 1;
    }

    @Override
    protected int get(FabricClientCommandSource source, ModConfigImpl modConfig, String config) {
        source.sendFeedback(Component.translatable("betterconfig.commands.config.get", config, modConfig.asString(config)));
        return 1;
    }
    ...
    protected int reset(FabricClientCommandSource source, ModConfigImpl modConfig, String config) {
        ...
        source.sendFeedback(Component.translatable("betterconfig.commands.config.reset", config, modConfig.asString(config)));
        return 1;
    }
    ...
    protected int set(FabricClientCommandSource source, ModConfigImpl modConfig, String config, Object value) throws CommandSyntaxException {
        ...
        source.sendFeedback(Component.translatable("betterconfig.commands.config.set", config, modConfig.asString(config)));
        return 1;
    }
